...
We chose the random forests implementated in the **ranger** R package for estimation of culture conditions, because of its speed and high accuracy. To find optimal values of hyperparameters, we performed a nested cross-validation of random forest classifiers. The inner loop was a 5-fold cross-validation and the outer loop was more demanding 3-fold cross-validation. 

We have optimized three hyperparameters related to the random forest algorithm: a number of variables to possibly split at in each node, a number of trees in the forest and a minimal node size. In the tuning procedure we also incorporated different data sources, levels of feature selection and n-gram lenghts.
...
In addition to the standard number of variables to possibly split at in each node for regresion tasks ($\frac{1}{3}$ of all considered features), we have also examined $\frac{1}{2}$ and $\frac{1}{4}$. In most cases, $\frac{1}{4}$ was the most optimal.
...
Below are included beeswarm plots for all parameters tuned for MethanoGram. The shape of points represents minimal node size and transparency distinguish between different mumber of variables to possibly split at in each node (mtry).
